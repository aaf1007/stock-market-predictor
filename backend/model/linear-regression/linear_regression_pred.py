...
def predict_price(ticker_symbol: str, days_ahead: int = 1, period_num: int = 60) -> float:
    ...
    stocks = build_data_set(training_stocks, days_ahead, period_num)
    ...
    X = stocks[["Close", "Volume", "Moving_Average_60", "Rolling_STD_60"]]
    y = stocks["Target"]
    ...
    latest_price_df["Moving_Average_60"] = latest_price_df["Close"].rolling(window=period_num-1).mean()
    latest_price_df["Rolling_STD_60"] = latest_price_df["Close"].rolling(window=period_num-1).std()

    latest_price_df = latest_price_df.dropna() # Drop NaN vals

    latest_price_features = latest_price_df[["Close", "Volume", "Moving_Average_60", "Rolling_STD_60"]].iloc[[-1]]
...
def build_data_set(stocks: list, pred_days: int, period: int = 60) -> pd.DataFrame:
    data_set = []
    ...
    for ticker in stocks:
        ...
        price["Target"] = price["Close"].shift(-pred_days) # Predicted Days

        # Features Engineering
        # TODO implement more features
        price["Moving_Average_60"] = price["Close"].rolling(window=period-1).mean() # 60 day moving average
        price["Rolling_STD_60"] = price["Close"].rolling(window=period-1).std() # 60 day rolling standard deviation
...
try:
    period = 7
    pred, err = predict_price("KO",1, period)
